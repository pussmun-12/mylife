...
function DiaryMainController($scope, $log, $timeout, $rootScope, $location, $navigate, dateService){
    ...
    $scope.saveFactBox = function(factbox,$event){
        ...
        if($scope.factboxImage){
            factbox.imageurl = 'http://localhost:62666/' + $scope.factboxImage.path;
    ...
    $scope.saveImageCaption = function(currentImage){
    ...
    $scope.addDayMainText = function(selection){
        ...
        if(selection === 'Lägg till text med rubrik'){
            ...
            $scope.factBoxes.push({title:'Titel', id:'factbox_' + ($scope.factBoxes.length + 1) + '_' + $scope.currentDate, subtitle: 'Underrubrik',
                text:'Text', imageurl: 'Ange bildadress på internet eller klicka på bild i bildspelet', showimageurlText: true, showTitle:true, showSubtitle:true, showLat:true,showLong:true,lat:'',long:'',
